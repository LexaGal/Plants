...
{
    public class ServiceScheduleMappingRepository : Repository<ServiceScheduleMapping>,
        IServiceScheduleMappingRepository
    {
        public override bool Edit(ServiceScheduleMapping value)
        {
            using (Context = new PlantingDb())
            {
                var serviceScheduleMapping = Context.ServiceSchedulesSet.Find(value.Id);
